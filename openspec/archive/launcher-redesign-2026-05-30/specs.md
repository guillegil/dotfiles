...
| D-LR-3 | Grid columns | 5 columns implemented (spec said 6) | Deliberate post-apply polish for width/density |
| D-LR-4 | History cap | 8 entries (spec said "min 10") | Design ADR-4 override; minimum surfaced limit is RECENT_LIMIT=5 |
| D-LR-5 | Terminal hint | `> terminal` mode hinted (spec said `⌘↵ terminal`) | New terminal-command mode added during implementation; Super+Enter still works |
